...
public class TreasureGame extends BaseGame implements KeyListener {
	...
	private void initInput() {
		...
		/*
		 * String gpName = im.getFirstGamepadName(); String Keyboard =
		 * im.getKeyboardName(); String mouseName = im.getMouseName();
		 * 
		 * c1c = new Camera3Pcontroller(camera1,p1,im,mouseName); c2c = new
		 * Camera3Pcontroller(camera2,p2,im,gpName); //Controls for P1
		 * ForwardAction mvForward = new ForwardAction(p1);
		 * im.associateAction(Keyboard,
		 * net.java.games.input.Component.Identifier.Key.S, mvForward,
		 * IInputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		 * 
		 * BackwardAction mvBackward = new BackwardAction(p1);
		 * im.associateAction(Keyboard,
		 * net.java.games.input.Component.Identifier.Key.W, mvBackward,
		 * IInputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		 * 
		 * LeftAction mvLeft = new LeftAction(p1); im.associateAction(Keyboard,
		 ...
		 * IInputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		 * 
		 * //Controls for P2 X_Action_Controller xControl = new
		 * X_Action_Controller(p2); im.associateAction(gpName,
		 ...
		 * IInputManager.INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN);
		 * 
		 * QuitAction stop = new QuitAction(this); im.associateAction(Keyboard,
	...
	private void createPlayerHUDs() {
		...
		// Player 2 score HUD
		player2ScoreString = new HUDString("Score = " + score2); // default is
																	// (0,0)
	...
	private void initGameObjects() {
		...
		// add some lines
		Line xLine = new Line(origin, new Point3D(100, 0, 0), new Color(255, 0,
				0), 1);
		addGameWorldObject(xLine);

		Line yLine = new Line(origin, new Point3D(0, 100, 0), new Color(0, 255,
				0), 1);
		addGameWorldObject(yLine);

		Line zLine = new Line(origin, new Point3D(0, 0, 100), new Color(0, 0,
				255), 1);
	...
	public void update(float elapsedTimeMS) {
		...
		if (tpt.getWorldBound().intersects(p1.getWorldBound())
		...
		if (cyl.getWorldBound().intersects(p1.getWorldBound())
		...
		if (sph.getWorldBound().intersects(p1.getWorldBound())
		...
		if (jade.getWorldBound().intersects(p1.getWorldBound())
		...
		if ((cyl.getWorldBound().intersects(p2.getWorldBound()))
				&& collidedWCylinder == false) {
			collidedWCylinder = true;
			numCrashes++;
			score2 += 500;
		...
		if ((sph.getWorldBound().intersects(p2.getWorldBound()))
				&& collidedWPyramid == false) {
			collidedWPyramid = true;
			System.out.println(sph.getWorldBound());
		...
		if ((jade.getWorldBound().intersects(p2.getWorldBound()))
				&& collidedWDiamond == false) {
			collidedWDiamond = true;
			numCrashes++;
			score2 += 1000;
	...
	/*
	 * protected void shutdown(){ display.close(); } protected void
	 * initSystem(){ //call a local method to create a DisplaySystem object
